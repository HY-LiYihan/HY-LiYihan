...
\titleformat{\section}
  {\large\bfseries\CJKfamily{zhhei}\color{accent}}
  {}
  {0pt}
  {}
  [\titlerule]
...
\newcolumntype{R}{>{\raggedleft\arraybackslash}p{0.34\textwidth}}
\newcolumntype{P}{>{\raggedleft\arraybackslash}p{0.13\textwidth}}
\newcolumntype{S}{>{\bfseries\raggedright\arraybackslash}p{0.13\textwidth}}

\newcommand{\name}[1]{%
  \begin{center}
    {\LARGE\bfseries\CJKfamily{zhhei} #1}\par
  \end{center}
}
...
\newcommand{\pubentry}[3]{%
  \begin{tabularx}{\textwidth}{@{}Y P@{}}
    \textbf{#1} & #2 \\
    #3 &
  \end{tabularx}
}

\newcommand{\pubentryreview}[4]{%
  \begin{tabularx}{\textwidth}{@{}Y P@{}}
    \textbf{#1} & #2 \\
    #3 & \textit{#4}
  \end{tabularx}
...
\newenvironment{cvitems}
  {\begin{itemize}[leftmargin=1.2em,itemsep=1.5pt,topsep=2pt,parsep=0pt,partopsep=0pt]}
  {\end{itemize}}
...
\cventry{香港科技大学（广州）}{2026年1月 -- 2026年4月}
{Thrust of Robotics and Autonomous Systems}{访问学者}

\textit{相关课程：大学计算机基础（95）、数学建模实践（92）、Matlab计算与仿真（90）、数据结构与算法实验（89）}
...
\pubentry
{RAPID Hand: A Robust, Affordable, Perception-Integrated, Dexterous Manipulation Platform\linebreak for Generalist Robot Autonomy}
{NeurIPS 2025}
...
{科研助理}{许人镜助理教授}
\begin{cvitems}
  \item 基于 Gaussian Splatting、语义分割与 agent 构建面向灵巧操作的 Real2Sim pipeline，复现真实场景并生成可交互仿真资产。
  \item 研究视触觉 WAM，探索多模态视触觉决策策略学习与评估流程，面向真实传感约束下的具身操作任务。
  \item 自主研发 UMI 遥操作设备，适配柔性夹取任务，并搭建通用操作数据集采集流程。
\end{cvitems}
...
{无人机研发实习生}{}
\begin{cvitems}
  \item 基于 ESP32 研发多机星型组网与 Mesh 通信，搭建数图传链路以支撑多机协同与地面站数据回传。
...
{科研助理}{成慧教授}
\begin{cvitems}
  \item 基于 RAPID Hand 结构与硬件开发，实现低延迟控制与视触觉同步，支撑全手灵巧操作实验。
  \item 开展视觉导航研究，基于无序图像建立拓扑导航地图并进行优化，提升开放场景导航鲁棒性。
  \item 开发端到端 RGB 到可通行性的模型，研究视觉可通行性估计与评估流程。
\end{cvitems}
...
\begin{cvitems}
  ...
  \item 组织举办校级比赛“智绘空间无人科技挑战赛”，统筹赛制设计、宣传报名、技术支持与现场执行。
...
\projectentry{Goldsmith：Gold-Loss 引导的智能体标注系统}
{2026年5月 -- 至今}
{第一作者；系统设计与实验}
{}
\begin{cvitems}
  \item 提出 Gold-Loss 引导的提示训练方法，将操作性定义作为可训练文本参数，用小规模专家 gold set 评估候选定义，并以结构化损失下降作为接受准则。
  \item 构建模型外智能体标注流水线，整合检索、格式修复、LLM judge 路由与人工复核，并在 NER、RE 与事件抽取（event extraction）任务上完成实验验证。
\end{cvitems}
...
\begin{cvitems}
  ...
  \item 开发 Rosetta 智能体标注流水线，包含概念引导、基于嵌入的 top-k 检索、LLM judge 路由、审阅反馈与 Prodigy 兼容 JSONL 导出。
\end{cvitems}
